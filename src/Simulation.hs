...
-- | the number of balls in the simualtion
count :: Int
count = 32

-- | the highest attainable speed in the simulation
maxSpeed :: Float
maxSpeed = 10.0

-- | return a list of n random ball states
initialState :: Int -> [BallState]
initialState 0 = []
initialState 1 = addBall []
initialState n = addBall (initialState (n - 1))

-- | add a ball to the state's list of balls
addBall :: [BallState] -> [BallState]
addBall state = state ++ [Ball { 
    pos = (0,  0), 
    vel = if (mod (length state) 2) == 0 then 
              (fromIntegral (length state), (fromIntegral (length state) * fromIntegral (length state)) * 0.32) 
          else 
              (-(fromIntegral (length state)), (fromIntegral (length state) * (fromIntegral (length state))) * 0.32),
    --vel = (0.0, 0.0),
    acc = (-0.001, 0.01), 
...
-- | advance the whole simulation the given number of seconds
...
-- | advance a single ball's simulation the given number of seconds
updateBall :: Float -> BallState -> BallState
updateBall seconds ball = 
    applyForces forces (Ball { pos = pos', vel = vel', acc = (0, 0), rad = rad ball, col = col ball })
    where 
...
-- | the friction of a circle with the given velocity
...
-- | the drag of a circle with the given radius
